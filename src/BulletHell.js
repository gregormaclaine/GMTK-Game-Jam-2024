...
class BulletHell {
  ...
  async pattern6() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      this.bullets.push(new Bullet(x, -100, 200, 0, 4));
  ...
  async level1() {
    await this.spawn_resources([0,0.5]);
    await this.pattern1();
    await timeout(1000);
    ...
    await this.spawn_resources([0,0.5]);
    await this.spawn_bullets(5, 150, 500, 50);
    
    await timeout(1000);
    await this.pattern1();
    await this.spawn_resources([0,0.5]);
    await this.spawn_bullets(5, 150, 300, 30);
